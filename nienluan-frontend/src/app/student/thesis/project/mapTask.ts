...
import { ThesisTask, ThesisTaskAttachment } from "@/types/thesisTask";
...
interface ApiThesisTaskAttachment {
    id: number;
    thesis_task_id: number;
    file_path: string;
    file_name: string;
}
...
interface ApiTask {
    ...
    thesis: ApiThesis;
    attachments: ApiThesisTaskAttachment[];
}
...
export function mapTask(data: ApiTask): ThesisTask {
    return {
        ...
        attachments: data.attachments.map((file): ThesisTaskAttachment => ({
            id: file.id,
            thesisTaskId: file.thesis_task_id,
