...
export async function thesisByTime(year: number, semester: number, reportPeriod: number) {
    const response = await api.get('/api/get-thesis-by-time', {
        params: {
            year: year,
            semester: semester,
...
export async function updateThesisTask(id: number, data: Partial<ThesisTask>) {
    const response = await api.post(`/api/update-thesis-task/${id}`, data);
    return response;
}
...
export const getThesisTask = async (params: {
    ...
    const res = await api.get("/api/get-thesis-task", { params });
...
export const filterThesisTask = async (params: {
    ...
    const res = await api.get("/api/search-thesis-task", { params });
