...
import { deleteMultiThesisTask, deleteThesisTask, filterThesisTask, getThesisTask, reviewThesisTask, thesisByTime } from './thesisTaskService';
...
const ProjectTaskPage = () => {
    ...
    const [isConfirmOpenReview, setIsConfirmOpenReview] = useState(false);
    ...
    const handleMultiDelete = async () => {
        try {
            ...
            const response = await deleteMultiThesisTask(selectedIds);
    ...
    const handleDelete = async (id: number) => {
        try {
            ...
            const response = await deleteThesisTask(id);
    ...
    const handleReview = async (id: number) => {
        try {
            ...
            const payload = {
                instructor_status: values.danhGia,
                instructor_note: values.ghiChu
            };
            const response = await reviewThesisTask(id, payload);
            ...
            setLoadingReview(false);
            const msg = error?.response?.data?.message || 'Đánh giá thất bại. Vui lòng thử lại.';
            toast.error(msg, {
                position: 'top-right',
            });
        }
    ...
    useEffect(() => {
        ...
        if (isFirstLoad.current && selectedYear && selectedSemester && selectedReportPeriod) {
            loadData({
                search: searchValue,
                year: selectedYear,
                semester: selectedSemester,
    ...
    return (
        <div>
            <div className="text-right mb-4 p-4 flex items-center justify-end bg-white rounded-md">
                <Space>
                    {hasPermission('delete:thesis-task') && (
                        <Popconfirm
                            title="Xác nhận xoá hàng loạt"
                            description={`Bạn có chắc chắn muốn xoá ${selectedIds.length} công việc?`}
            ...
            <Modal
                ...
                open={isConfirmOpenReview}
                onOk={handleConfirmReview}
                confirmLoading={loadingReview}
                onCancel={() => {
                    setIsConfirmOpenReview(false);
                    form.resetFields();
                }
