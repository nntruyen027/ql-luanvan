...
import TeacherModalForm from './TeacherModalForm';
import { Teacher } from "@/types/user";
import { PageData } from '@/components/Table';
import { createTeacherAccount, deleteMultiTeacher, deleteTeacher, filterTeacher, getMajorListOptions, getRolesListOption, getTeacherList, resetTeacherAccount } from './teacherService';
...
const Page = () => {
    ...
    const userRole = typeof window !== 'undefined' ? JSON.parse(localStorage.getItem('roles') || '{}') : {};
    const roleName = userRole?.name || 'Default';
    const [form] = Form.useForm();
    const [data, setData] = useState<PageData<Teacher>>({
        content: [],
        totalElements: 0,
        page: 1,
        size: 10,
    ...
    const handleResetAccount = async (id: number) => {
        try {
            ...
            const response = await resetTeacherAccount(id);
    ...
    const handleCreateAccount = async (id: number) => {
        try {
            ...
            const response = await createTeacherAccount(id, formData);
    ...
    const handleMultiDelete = async () => {
        try {
            ...
            const response = await deleteMultiTeacher(selectedIds);
    ...
    const loadData = async ({
        ...
        let response = await getTeacherList({
            page,
            per_page: pageSize,
            sort_by,
            sort_order,
    ...
    const loadRolesData = async () => {
        ...
        if (roleName === 'Super_admin' || 'super_admin') {
    ...
    const searchTeacher = async () => {
        try {
            ...
            const response = await filterTeacher({ page, per_page: 10, search, major });
            setData(response);
    ...
    if (!hasPermission('access:faculty-staff')) {
    ...
    return (
        <div>
            <div className="text-right mb-4 p-4 flex items-center justify-end bg-white rounded-md">
                <Space>
                    {hasPermission('delete:faculty-staff') && (
                        <Popconfirm
                            title="Xác nhận xoá hàng loạt"
                            description={`Bạn có chắc chắn muốn xoá ${selectedIds.length} cán bộ?`}
                    ...
                    {hasPermission('add:faculty-staff') && (
            ...
            <div className="bg-white p-4 rounded-md">
                <Space style={{ marginBottom: 16, width: '100%' }}>
                    ...
                    <Button type="primary" icon={<SearchOutlined />} loading={loadingSearch} onClick={searchTeacher}>
                        Tìm kiếm
                    </Button>
                </Space>
            ...
            >
                <p>Bạn có chắc chắn muốn xoá cán bộ này không?</p>
            </Modal>
            ...
            <Modal
                ...
                cancelText="Huỷ"
            >
                <p>Bạn có chắc chắn muốn đặt lại mật khẩu tài khoản của cán bộ này không?</p>
            </Modal>
